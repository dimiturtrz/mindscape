...
class WorkloadConfusion:
    ...
    def main(cls):
        ...
        zerorow = conf[0]
        zero_to_load_pct = 100 * (zerorow[1] + zerorow[2]) / zerorow.sum()
        load_to_zero_pct = 100 * conf[1:, 0].sum() / conf[1:].sum()
        logger.info(
            f"\n  0-vs-load separability (is rest confused with load?): "
            f"0-back called load {zero_to_load_pct:.0f}% · load called 0-back {load_to_zero_pct:.0f}%"
        )
        logger.info(f"  2-vs-3 accuracy among true-load trials: {acc_2v3:.3f}")
        logger.info(
            "  -> if 0-vs-load leaks, fusion (fNIRS strong there) can fix it; if clean, the loss is all 2-vs-3."
        )


if __name__ == "__main__":
    WorkloadConfusion.main()
