...
def _cv(X, y, groups, grouped: bool):
...
def main():
    print(f"fNIRS cleaner ablation · Shin n-back · fixed decoder fnirs_lda · {len(_SEEDS)}x{_K}-fold · chance 0.333\n")
    ...
    for name, spec in _ARMS:
        ...
        wa, ws, wk = _cv(X, y, groups, grouped=False)
        ca, cs, ck = _cv(X, y, groups, grouped=True)
