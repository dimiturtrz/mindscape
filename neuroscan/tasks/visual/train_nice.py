...
from neuroscan.tasks.visual.epoch_dataset import EpochDataset
...
class _EvalResult(TypedDict, total=False):
    """Evaluation result dict structure from evaluate() method."""

    single_trial: dict[int, float]
    single_trial_ci: dict[int, object]
    concept_avg: dict[int, float]
    continuous: dict[str, float]
    retrieval_metrics: dict[str, float]
    emb_mse: float
    single_trial_hits: dict[int, object]


class _TrainResult(TypedDict, total=False):
    """Training result dict structure from train() method."""
...
class TrainNice:
    ...
    @staticmethod
    def _clip_targets(
        image_files: Shaped[np.ndarray, "n"], split: str, target: str = "vitb32"
    ...
    @staticmethod
    @torch.no_grad()
    def evaluate(encoder: torch.nn.Module, data: RetrievalSet, device: str, batch: int = _EVAL_BATCH) -> _EvalResult:
        """Single-trial + concept-averaged retrieval top-1/5 against a per-concept prototype bank."""
        encoder.eval()
        with torch.autocast("cuda", dtype=torch.bfloat16, enabled=(device == "cuda")):
            embedded = torch.cat(
                [
                    encoder(torch.tensor(data.eeg[i : i + batch]).to(device)).float().cpu()
                    for i in range(0, len(data.eeg), batch)
                ]
            )  # [N,512] normalized (back to fp32)
        ...
        averaged = torch.stack(
            [torch.nn.functional.normalize(embedded[labels == c].mean(0), dim=-1) for c in range(n_concepts)]
        )
        ...
        return cast(
            ...
            {
                "single_trial": single,
                "single_trial_ci": single_ci,
                "concept_avg": concept_avg,
                "continuous": continuous,
                "retrieval_metrics": rank_metrics,
                "emb_mse": emb_mse,
                "single_trial_hits": {k: h.tolist() for k, h in hits.items()},
            },
        )  # persisted for paired delta (s1t2)
    ...
    @staticmethod
    def _concept_neighbor_groups(
        targets: Float[np.ndarray, "n d"], concept_ids: Int[np.ndarray, "n"], k: int
    ) -> dict[int, list[int]]:
    ...
    def _epoch_steps(
        data: _FitArrays,
        neighbor_groups: dict[int, list[int]] | None,
        cfg: TrainConfig,
        ...
        if cfg.sampling in ("balanced", "clip_hard"):
            ...
            pos_batches = (
                Sampling.clip_hard_batches(data.concept[fit], neighbor_groups, spec, rng)  # type: ignore[arg-type]
                if cfg.sampling == "clip_hard"
                else Sampling.balanced_batches(data.concept[fit], spec, rng)
            )
            ...
            return (
                (
                    torch.from_numpy(data.eeg[fit[pos]]),
                    torch.from_numpy(data.targets[fit[pos]]),
                    torch.from_numpy(data.subject[fit[pos]]),
                )
        ...
        if cfg.sampling == "stratified":
            return (
                (
                    torch.from_numpy(data.eeg[epoch_idx[pos]]),
                    torch.from_numpy(data.targets[epoch_idx[pos]]),
                    torch.from_numpy(data.subject[epoch_idx[pos]]),
                )
                for pos in Sampling.stratified_batches(data.concept[epoch_idx], cfg.batch, rng)
                if len(pos) > 1
            )
        return DataLoader(
            EpochDataset(data.eeg, data.targets, epoch_idx, data.subject),
    ...
    def _build_optim(spec: EncoderSpec, n_subjects: int, cfg: TrainConfig, device: str):
        ...
        groups = (
            make_groups(cfg.lr, cfg.backbone_lr_scale)
            if make_groups
    ...
    @staticmethod
    def _geo_laplacian(encoder: torch.nn.Module, data: TrainData, cfg: TrainConfig, device: str):
    ...
    def train_encoder(data: TrainData, cfg: TrainConfig, device: str):
        ...
        logger.info(
            f"early-stop val: {len(val_bank)} held-out train concepts, {int(val_mask.sum())} epochs; "
        ...
        tr = _StepCtx(
            encoder, optimizer, logit_scale, discriminator, TrainNice._geo_laplacian(encoder, data, cfg, device)
        )  # spatial-smoothness prior (bd 1x0)
        ...
        for epoch in range(cfg.epochs):
            lam = (
                TrainNice._dann_lambda(epoch / max(1, cfg.epochs - 1), cfg.adv_lambda)
                if cfg.adv_lambda_ramp
                else cfg.adv_lambda
            )
                ...
                Tracking.metrics(
                    {
                        ...
                        "sec_per_epoch": train_s,
                    },
                    step=epoch,
                )  # per-epoch curve + speed in mlflow
                if epoch % 5 == 0 or epoch == cfg.epochs - 1:
                    logger.info(
                        f"ep {epoch:3d}  loss {total_loss / max(1, n_batches):.3f}  "
        ...
        return encoder, {"best_val_epoch": stopper.best_step, "val_top1": stopper.best_metric, "epochs_run": epoch + 1}

    @staticmethod
    def train(train_subjects: list[int], test_subject: int, cfg: TrainConfig) -> _TrainResult:
        ...
        logger.info(
            f"train {train_eeg.shape} -> CLIP {train_targets.shape} | "
        ...
        with Tracking.run(
            "mindscape-perception",
            f"{cfg.model}_{regime}_test{test_subject}_s{cfg.seed}",
            params=cast(dict[str, object], params),
            tags=cast(dict[str, object], tags),
        ):
            ...
            Tracking.metrics(
                {
                    "test_single_top1": single[1],
                    "test_single_top5": single[5],
                    "test_concept_top1": concept[1],
                    "test_concept_top5": concept[5],
                    "best_val_top1": stats["val_top1"],
                    **{f"test_{k}": v for k, v in test["continuous"].items()},  # angular error (bd 2y7k)
                    **{f"test_{k}": v for k, v in test["retrieval_metrics"].items()},
                }
            )  # rank-aware (bd 7tl)
            rm = test["retrieval_metrics"]
            logger.info(
                f"test5 single-top1 {single[1] * 100:.2f}%  MRR {rm['mrr']:.3f}  "
            ...
            torch.save(
                {
                    ...
                    "n_times": train_eeg.shape[2],
                    "embed_dim": train_targets.shape[1],
...
def main():
    ...
    overrides = {k: v for k, v in vars(args).items() if k in TrainConfig.model_fields and v is not None}
