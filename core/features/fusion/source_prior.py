...
class SourcePrior:
    """fNIRS-informed EEG source imaging (bd 4so) — the weighted minimum-norm inverse regularized by an fNIRS
    ...
    def parcels_from_leadfield(epochs: Float[np.ndarray, "n ch t"], leadfield: Float[np.ndarray, "ch s"],
        ...
        k = SourcePrior.weighted_min_norm_inverse(leadfield, w, snr)
