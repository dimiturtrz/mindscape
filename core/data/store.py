...
class Store:
    """Consolidated epoch store — the free helpers folded in as staticmethods (public names kept). Turns the
    ...
    def build(name: str, cfg: EpochCfg, *, rebuild: bool = False) -> Path:
        ...
        for sub in adapter.subjects():
            ...
            rows.extend(Store._rows_for_subject(name, sub, npz, label_names))
...
def main():
    ...
    args = ap.parse_args()
    df = Store.load(args.name, EpochCfg())
