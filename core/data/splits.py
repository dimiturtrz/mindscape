...
class Splits:
    """Train/val/test splits as criteria over the epoch cloud — the free helpers folded in as staticmethods
    (public names kept). The criteria self-document what was held out (the leakage-control story)."""
    ...
    def make_split(meta: pl.DataFrame, spec: SplitSpec | None = None
        ...
        train, val = Splits._val_carve(rest, spec.val_frac, spec.seed)
    ...
    def within_subject(meta: pl.DataFrame, subject: str, test_sessions=(), val_frac: float = 0.2,
        ...
        if test_sessions:
            return Splits.make_split(one, SplitSpec(test_sessions=tuple(test_sessions),
        ...
        train, val = Splits._val_carve(rest, val_frac, seed)
        return train, val, test
