...
class OnnxExport:
    """ONNX export + INT8 quantization + parity/size/latency — the free helpers folded in as staticmethods
    ...
    def parity(net, onnx_path: str | Path, X_std: np.ndarray, device: str = "cpu") -> float:
        ...
        return float(np.abs(tlog - OnnxExport.run(onnx_path, X_std)).max())
