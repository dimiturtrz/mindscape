"""Import-graph architecture diagnostic (bd 2r9) — the degree>0 view vulture (degree-0 dead code) can't
...
run after big structural changes. IMPORT-level only: function-level lies on the neuroscan.models.get_method
registry dispatch. Dict-registered variants show as real edges, so a registry hub's fan-in reads
expected-high — the signal you want is fan-in/out on LOGIC modules. `python -m devtools.graph`.
"""
