"""Import-graph architecture diagnostic + fitness gate (bd 2r9 / 3nn). grimp builds the honest import graph;
networkx ranks structure and `--assert` turns the measurable properties into a RATCHETED gate (the metric
arch axis import-linter's categorical layer contracts can't express):
...
`python -m devtools.graph` | `... --assert`.
"""
...
# Fitness thresholds (overridable in pyproject [tool.structure]). Defaults chosen against the CURRENT graph so
# the blocking rules start CLEAN and ratchet: fan-in&out both>8 (0 today), file>750 (max 373 today), cycles (0
# today). The line FLOOR is OFF (file_min=0) — no honest floor exists, the small files are SSOT/strategy/
...
_STRUCTURAL = ("__init__.py", "__main__.py")        # package plumbing — exempt from the line floor + mirror
...
def _undersized(files: list[tuple[str, int]], mn: int) -> list[str]:
    if mn <= 0:                                     # floor OFF (no honest line floor — small files are SSOT/strategy leaves)
        return []
