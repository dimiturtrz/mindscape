"""LCOM cohesion detector: Chidamber-Kemerer's Lack of Cohesion of Methods, the LCOM4 (Hitz-Montazeri)
variant — a class's methods form a graph (edge = two methods share an instance field or one calls the
other); the number of CONNECTED COMPONENTS is LCOM4. LCOM4 == 1 is cohesive; LCOM4 >= 2 means the methods
split into groups touching disjoint state = the class is really N classes fused (extract each component).
Formalizes what `state_candidates` eyeballed: where that ranks namespace bags by shared params, this ranks
CONSTRUCTED classes by internal split.
...
(an adapter's get_data/subjects, a Method's fit/score) and a null-object stub legitimately have methods
...
survives is a CONCRETE, STATEFUL class whose behaviour genuinely splits into disjoint-state groups.
    ...
    python -m devtools.lcom core neuroscan
"""
