...
class WindowedConfig(BaseModel):
    """WindowedFnirs knobs. `win_s`/`hop_s` = sub-window length and stride in seconds; `fs` converts to samples
    (Shin fNIRS = 10 Hz); `families` = per-window descriptors; `aggregate` (`concat`|`mean`|`max`|`lse`) is the
    design axis; `add_position` appends the normalized sub-window centre to the pooled stage-1's features
    (ignored by `concat`, which is position-aware already).
...
class WindowedFnirs(Baseline):
    """Sub-window the hemodynamic response and combine the windows into a block decision via `config.aggregate`
    (`concat`|`mean`|`max`|`lse`), configured by `WindowedConfig`."""

    def __init__(self, config: WindowedConfig | None = None):
        self.cfg = config or WindowedConfig()
        if self.cfg.aggregate not in _AGGREGATES:
            raise ValueError(f"aggregate must be one of {_AGGREGATES}, got {self.cfg.aggregate!r}")

    # --- windowing -------------------------------------------------------------------------------------
    def _starts(self, t: int) -> list[int]:
        """Sub-window start indices for a length-`t` block (clamped so a short block still yields ≥1)."""
        L = max(1, int(round(self.cfg.win_s * self.cfg.fs)))
        hop = max(1, int(round(self.cfg.hop_s * self.cfg.fs)))
        if t <= L:
            return [0]
    ...
    def _window_feats(self, X: np.ndarray) -> np.ndarray:
        ...
        for s in starts:
            ...
            feats = np.concatenate([np.asarray(FNIRS_FEATURE_FNS[f](win), dtype=np.float64)
                                    for f in self.cfg.families], axis=1)  # [n, K*ch]
            if self.cfg.add_position and self.cfg.aggregate != "concat":
    ...
    def fit(self, X, y):
        ...
        if self.cfg.aggregate == "concat":
            ...
            if self.cfg.aggregate in ("max", "lse") and len(self.classes_) < _MIL_MIN_CLASSES:
                ...
                raise ValueError(f"aggregate={self.cfg.aggregate!r} needs >=3 classes (got {len(self.classes_)}); "
    ...
    def predict_proba(self, X):
        ...
        if self.cfg.aggregate == "mean":                                 # average per-window probabilities
        ...
        pooled = s.max(axis=1) if self.cfg.aggregate == "max" else _logsumexp(s, axis=1)
