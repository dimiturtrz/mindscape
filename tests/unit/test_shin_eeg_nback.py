...
def test_epoch_windows_and_drops_edges():
    ...
    y = np.array([0, 1, 2])
    X, ye = m._epoch(cont, onsets, y, fs, tmin=0.0, tmax=2.0)   # 2s * 100Hz = 200 samples
...
def test_epoch_all_dropped_returns_empty():
    cont = np.zeros((28, 100))
    X, ye = m._epoch(cont, np.array([90]), np.array([1]), 100.0, tmin=0.0, tmax=2.0)  # 200 > 100
