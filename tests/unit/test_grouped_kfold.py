...
def test_grouped_kfold_partitions_subjects_once():
    ...
    assert len(folds) == 5
    test_subs = [set(te["subject"].unique().to_list()) for _, _, _, te in folds]
...
def test_grouped_kfold_train_test_disjoint():
    for _, tr, _val, te in splits.grouped_kfold(_meta(26), k=5):
        ...
        assert tr_s | te_s == {str(s) for s in range(26)}           # together they cover all subjects
