...
def test_render_single_value():
    assert SyncNumbers._render(_RUNS, "csp_lda.acc") == "0.598"


def test_render_signed_difference():
    assert SyncNumbers._render(_RUNS, "csp_lda.acc-eegnet.acc") == "+0.086"    # within->cross gap, unicode-safe sign
    assert SyncNumbers._render(_RUNS, "eegnet.acc-csp_lda.acc").startswith("−")   # negative -> unicode minus


def test_lookup_raises_on_bad_term_missing_run_and_field():
    with pytest.raises(KeyError):
        SyncNumbers._lookup(_RUNS, "no_dot_field")
    with pytest.raises(KeyError):
        SyncNumbers._lookup(_RUNS, "ghost.acc")
    with pytest.raises(KeyError):
        SyncNumbers._lookup(_RUNS, "csp_lda.ece")                       # run exists, field absent


def test_markers_extracts_expr_current_and_rendered():
    text = "acc **<!--r:csp_lda.acc-->0.111<!--/r-->** end"
    marks = SyncNumbers._markers(_RUNS, text)
    assert len(marks) == 1
