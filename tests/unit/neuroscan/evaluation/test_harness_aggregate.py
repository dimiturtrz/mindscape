"""harness.aggregate — the pure fold->metrics spine (fold-mean + pooled), and cross_subject_kfold folds.

store.gather (the disk read) is stubbed so only the aggregation LOGIC is under test: each synthetic fold
yields known (X, y); a trivial perfect scorer makes acc/kappa exact and the confusion diagonal."""
import numpy as np
import polars as pl
