...
from baselines.fnirs.windowed import _AGGREGATES, WindowedConfig, WindowedFnirs
...
def test_predict_proba_contract(agg):
    ...
    p = WindowedFnirs(WindowedConfig(win_s=2.0, hop_s=0.5, fs=10.0, aggregate=agg)).fit(X, y).predict_proba(X)
...
def test_concat_captures_ordered_shape():
    ...
    acc_win = _acc(WindowedFnirs(WindowedConfig(win_s=2.0, hop_s=0.5, fs=10.0, aggregate="concat")), X, y, cut)
...
def test_short_block_yields_one_window():
    clf = WindowedFnirs(WindowedConfig(win_s=6.0, hop_s=1.0, fs=10.0))
...
def test_bad_aggregate_rejected():
    with pytest.raises(ValueError):
        WindowedFnirs(WindowedConfig(aggregate="nope"))
...
def test_mil_pooling_rejects_binary(agg):
    ...
    with pytest.raises(ValueError):                                      # MIL score-pool needs a per-class axis
        WindowedFnirs(WindowedConfig(win_s=2.0, hop_s=0.5, fs=10.0, aggregate=agg)).fit(Xb, yb)
