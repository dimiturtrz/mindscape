...
from core.data.fnirs.base import FnirsCfg, FnirsEpochs
...
def test_bandpass_removes_dc_and_drift():
    ...
    y = Signal.bandpass(x[None, :], 0.01, 0.2, fs)[0]
...
def test_epoch_blocks_extracts_correct_samples():
    ...
    X, ye = FnirsEpochs.epoch_blocks(cont, np.array([300]), np.array([0]), fs, cfg)
...
def test_epoch_blocks_drops_out_of_range():
    ...
    X, ye = FnirsEpochs.epoch_blocks(cont, np.array([290]), np.array([1]), 10.0, FnirsCfg(tmin=-2, tmax=8))
    assert len(ye) == 0
...
def test_fnirs_features_shape_and_decodes_amplitude_signal():
    ...
    feats = Amplitude.amplitude_features(X)
