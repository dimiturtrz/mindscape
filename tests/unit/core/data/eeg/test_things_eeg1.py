...
from core.data.eeg.things_eeg1 import ThingsEeg1, ThingsEeg1EpochCfg
...
def test_drop_targets_and_validation_filtering():
    ...
    _, concept, _ = ThingsEeg1.epochs_from_events(eeg, 1000.0, ev, ThingsEeg1EpochCfg(tmax=0.1, resample=0))
    assert list(concept) == ["a", "d"]
    # include_validation keeps the test stim; keep_targets keeps the fixation-colour trial
    _, concept2, _ = ThingsEeg1.epochs_from_events(eeg, 1000.0, ev,
...
def test_row_mask_absent_flag_columns_keep_all():
    ...
    assert ThingsEeg1._row_mask(ev, ThingsEeg1EpochCfg()).all()
...
def test_overrunning_window_dropped():
    ...
    epochs, concept, _ = ThingsEeg1.epochs_from_events(eeg, 1000.0, ev, ThingsEeg1EpochCfg(tmax=0.2, resample=0))
...
def test_onset_units_guard_raises_on_out_of_range():
    ...
    with pytest.raises(ValueError, match="onset units"):
        ThingsEeg1.epochs_from_events(eeg, 1000.0, ev, ThingsEeg1EpochCfg())


def test_resample_changes_time_length():
    eeg = _ramp_eeg()
    ev = _events([0.5], ["a"], ["1"])
    epochs, _, _ = ThingsEeg1.epochs_from_events(eeg, 1000.0, ev, ThingsEeg1EpochCfg(tmax=0.2, resample=250.0))
