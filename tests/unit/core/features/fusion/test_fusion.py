...
def test_cbsi_cancels_common_mode_keeps_anticorrelated():
    ...
    common = bc.Chromophore.cbsi_neural(s, s.copy())     # HbO == HbR (motion/systemic) -> ~0
    anti = bc.Chromophore.cbsi_neural(s, -s.copy())      # HbO == −HbR (neural activation) -> preserved
...
def test_estimate_coupling_recovers_planted_lag():
    ...
    g = bc.Coupling._gamma_kernel(tk, peak=6.0, width=3.0)
    ...
    lag, decay, beta = bc.Coupling.estimate_coupling(drive, resp, fs)
...
def test_build_tensor_contract():
    ...
    X = bc.BrainCamera.build_tensor(bc.PairedModalities(Xe, Xf, pos_e, pos_f), grid=grid,
...
def test_fused_node_series_eeg_format():
    ...
    pos_f = rng.uniform(-0.8, 0.8, (6, 2))
    joint, coupling = bc.BrainCamera.fused_node_series(bc.PairedModalities(Xe, Xf, pos_e, pos_f),
                                                       series=bc.SeriesConfig(fps=10.0, t_end=20.0))
...
def test_channel_series_fixed_lag_override_skips_estimation():
    ...
    eeg, neural, t_dst, coupling = bc.Series.channel_series(
...
def test_csd_transform_preserves_shape():
    ...
    out = bc.CSD.csd_transform(Xe, ch, 100.0)
...
def test_coverage_is_local_and_bounded():
    ...
    cov = bc.BrainCamera.coverage_map(pos_e, pos_f, grid=16)
