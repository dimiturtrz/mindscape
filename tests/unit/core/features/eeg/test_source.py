...
def test_cache_key_deterministic_and_input_sensitive():
    cfg = SourceConfig()
    chs = ["C3", "Cz", "C4"]
    assert Source._cache_key(chs, 250.0, cfg) == Source._cache_key(chs, 250.0, cfg)          # deterministic
    assert Source._cache_key(chs, 250.0, cfg) != Source._cache_key(chs, 128.0, cfg)          # sfreq matters
    assert Source._cache_key(chs, 250.0, cfg) != Source._cache_key(["C3", "C4"], 250.0, cfg)  # montage matters
    assert Source._cache_key(chs, 250.0, cfg) != Source._cache_key(chs, 250.0, SourceConfig(spacing="oct6"))  # cfg matters


def test_montage_info_rejects_unknown_channels():
    with pytest.raises(ValueError, match="not in standard_1005"):
        Source._montage_info(["C3", "NOTACHAN"], 250.0)


def test_montage_info_builds_average_referenced_eeg_info():
    info = Source._montage_info(["C3", "Cz", "C4"], 250.0)
