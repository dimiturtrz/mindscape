...
from core.features import SubjectNorm
...
def test_each_subject_standardized_by_its_own_stats():
    F, g = _two_subjects()
    Z = SubjectNorm.zscore_per_subject(F, g)
    for s in ("A", "B"):
        m = g == s
...
def test_held_out_subject_uses_its_own_stats_not_others():
    ...
    ga = np.array(["A"] * 20)
    Za_alone = SubjectNorm.zscore_per_subject(A, ga)
    Z_joint = SubjectNorm.zscore_per_subject(np.vstack([A, B]), np.array(["A"] * 20 + ["B"] * 20))
...
def test_zero_variance_feature_is_finite():
    ...
    Z = SubjectNorm.zscore_per_subject(F, np.zeros(10, int))
